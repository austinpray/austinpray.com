(function(){
	...
	var c1=[-130, 40], c2=[20, 220], c3=[60, 260]; //Column positions of labels.
...
loading.setAttribute('id', 'loadingIndicator');
loading.className = 'loading-indicator';
loading.appendChild(
  document.createTextNode('Loading...')
);
document.getElementById('graph').appendChild(loading);

function parseGithubStream (json) {
  var actionsMap = {
    CreateEvent: 'Create',
    DeleteEvent: 'Delete',
    ...
    PullRequestReviewCommentEvent: 'Review Pull Request',
    CommentEvent: 'Comment',
    IssuesEvent: 'New Issue',
    PushEvent: 'Push',
    WatchEvent: 'Watch Repo'
  };
  return json.map(function (node) {
    var type = actionsMap[node.type] || node.type;
    if(node.type === 'CreateEvent') {
      type += ' ' + node.payload.ref_type.charAt(0).toUpperCase() + node.payload.ref_type.slice(1);
    }
    return [type, node.repo.name, 1, 1];
  });
}
...
function buildGraph () {
  if(this.status === 200) {
    ...
    var width = 740, height = 610, margin ={b:0, t:40, l:170, r:50};

    var svg = d3.select("#graph")
    .append("svg").attr('width',width).attr('height',(height+margin.b+margin.t))
    .append("g").attr("transform","translate("+ margin.l+","+margin.t+")");

    var github_data = parseGithubStream(JSON.parse(this.responseText));

    var data = [ 
      {data:bP.partData(github_data,2), id:'GithubActivity', header:["Activity","Repo", "Github Activity"]},
    ];

    bP.draw(data, svg);
  } else {
    loading.textContent = 'having trouble reaching github.';
  }
}

var oReq = new XMLHttpRequest();
oReq.onload = buildGraph;
oReq.onerror = function () {
  loading.textContent = 'having trouble reaching github.';
};
oReq.open("get", "https://d1v9vuhvuxb9ex.cloudfront.net/users/austinpray/events/public", true);
oReq.send();
